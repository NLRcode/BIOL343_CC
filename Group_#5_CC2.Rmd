...
positpal::start("Group_#5_CC2.Rmd")
```
...
1. **Noah Lu** — 20483156
    + [BIOL343_CC](https://github.com/NLRcode/BIOL343_CC)
2. **Allie Goodyear** — 20443234
    + [BIOL343_CC](https://github.com/alliegoodyear/BIOL343_CC2)
3. **Zeshan Ghuman** — 20479122
    + [BIOL343_CC](https://github.com/Potato9715/BIOL343_CC2)
4. **Coralie Spakowski** — 20471806
    + [BIOL343_CC](https://github.com/coraliespakowski/BIOL343_CC)
5. **Jake Mayo** — 20186062
    + [BIOL343.CC](https://github.com/jake-mayo/BIOL343.CC)
6. **Iryna Ozimek** — 20508541
    + [BIOL343_CC](https://github.com/irynaoz323/BIOL343.CC)
...
----------------|--------------------------------------------------|------
Population Type |Indicates whether the species is sexual or asexual|NA
Ovary Width     |A measure of the width of the ovaries             |mm
Ovary Height    |A measure of the height of the ovaries            |mm
Flower Number   |The average of the flower numbers                 |count
...
names(cc1_data) <- c("population_type", "population_name", "latitude", "longitude", "ovary_width", "ovary_height", "avg_aug_temp", "flower_number")
cc1_data[,7] <- as.numeric(cc1_data[,7])
...
$$ OvaryArea = OvaryWidth \times OvaryHeight$$
Unit check: $mm^2 = mm \times mm$
...
area_means <- aggregate(cc1_data$ovary_area, list(PopulationType = cc1_data$population_type), mean)
...
```{r area-table, echo=F}
library(knitr)
kable(area_means, caption = "Summary Table comparing the ovary size area means between asexual and sexual types")
```
...
library(ggplot2)
ggplot(aes(x=ovary_area),data=cc1_data) + geom_histogram(binwidth=1) + theme_classic() +  facet_wrap(.~population_type) + xlab('Ovary Area (mm^2)') + labs(title= 'Comparison in Ovary Area by Population Type in Solanum pimpinellifolium')
```
...
![Image of Solanum pimpinellifolium](./image/Hydroponic_g13.jpg)
